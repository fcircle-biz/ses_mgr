...
- **FileRepository**: Spring Data JDBCを活用したリポジトリ
...
- Spring Data JDBCを活用したメタデータ管理実装
...
|-----|------|
| ファイルメタデータ | ファイルに関する説明的な情報（Spring Data JDBCによって管理されるエンティティ） |
...
| 業務エンティティ | ファイルが関連づけられるビジネスオブジェクト（Spring Data JDBCエンティティとして表現） |
