...
public interface FileService {
    ...
    @PreAuthorize("hasAuthority('FILE_READ')")
    @Transactional(readOnly = true)
    List<FileMetadataDTO> searchFiles(FileSearchCriteria criteria, int page, int size, String sortBy);
...
public class FileController {
    ...
    @GetMapping("/search")
    public List<FileMetadataDTO> searchFiles(
            @ModelAttribute FileSearchCriteria criteria,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size,
            @RequestParam(defaultValue = "createdAt") String sortBy) {
        return fileService.searchFiles(criteria, page, size, sortBy);
    }
}
```
...
public class FileSearchCriteria {
    ...
    public String toWhereClause() {
        ...
        if (StringUtils.hasText(moduleCode)) {
            conditions.add("module_code = :moduleCode");
        }
...
| メタデータ検索 | 95%のクエリを500ms以内に応答 | Spring Data JDBC最適化クエリ、@Cacheable |
...
### 9.2 ファイル検索例（Spring Data JDBC + JDBCTemplate）
...
public class FileSearchController {
    ...
    public ResponseEntity<List<FileMetadataDTO>> searchFiles(
        ...
        // Spring Data JDBCでは独自のページング処理を実装
        List<FileMetadataDTO> results = fileService.searchFiles(criteria, page, size, sortBy, sortDirection);
...
```java
@Configuration
@EnableJdbcRepositories(basePackages = "com.example.sesmgr.filemanagement.repository")
