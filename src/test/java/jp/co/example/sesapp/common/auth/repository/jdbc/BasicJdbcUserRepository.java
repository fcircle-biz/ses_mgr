...
/**
 * テスト用のシンプルなUserRepositoryの実装
 */
public class BasicJdbcUserRepository implements UserRepository {
    ...
    public BasicJdbcUserRepository(JdbcTemplate jdbcTemplate, NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
    ...
    public Optional<User> findById(UUID userId) {
        try {
            ...
            System.err.println("findById エラー: " + e.getMessage());
            e.printStackTrace();
            return Optional.empty();
        }
    }
    ...
    private User updateUser(User user) {
        String sql = "UPDATE auth.users SET " +
                "username = ?, email = ?, password_hash = ?, first_name = ?, last_name = ?, department_id = CAST(? AS UUID), " +
                "is_enabled = ?, is_account_locked = ?, account_expire_date = ?, credentials_expire_date = ?, " +
